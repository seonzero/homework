...
# 3. <a> 태그 개수 세기
links = soup.find_all('a')  # 모든 <a> 태그를 찾습니다. - 모든 태그를 찾아야 하므로: find_all
print("\n== 문제 3. <a> 태그 개수 세기 ==")
print("답: 링크 개수:", len(links))  # links의 길이를 출력합니다.
print("설명: find_all() 메소드를 사용하여 모든 <a> 태그를 찾고, 그 개수를 계산했습니다.")


# 4. 외부 링크 필터링 후 (`href` 속성에 'http' 또는 'https'가 포함된 링크)
# 첫 5개의 href 속성 출력
external_links = [link.get('href') for link in links if link.get('href') 
                  and ('http' in link.get('href') or 'https' in link.get('href'))]  
                    # 외부 링크만 필터링합니다.
