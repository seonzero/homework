...
response = requests._____(url, verify=False)  # SSL 인증서 검증 비활성화 후 가져오기
print("문제 1. 웹페이지 가져오기")
print("답: 상태 코드:", response._________)  # 응답의 상태 코드를 출력합니다.
print("설명: 상태 코드 200은 웹페이지가 성공적으로 로드되었음을 의미합니다.")

# 2. <title> 태그 내용 추출
soup = BeautifulSoup(response.____, '_________')  # HTML 파서를 지정합니다.
title = soup._____(_____)  # <title> 태그를 찾습니다.
print("\n문제 2. <title> 태그 내용 추출")
print("답: 웹페이지 제목:", _____ if title else "제목 없음")  # title의 텍스트를 출력합니다.
print("설명: BeautifulSoup을 사용하여 HTML에서 <title> 태그의 내용을 추출했습니다.")

# 3. <a> 태그 개수 세기
links = soup.________('a')  # 모든 <a> 태그를 찾습니다.
print("\n문제 3. <a> 태그 개수 세기")
print("답: 링크 개수:", _____(links))  # links의 길이를 출력합니다.
...
print("답:", external_links[_____:_____])  # 첫 5개의 외부 링크를 출력합니다.
print("설명: href 속성에 'http' 또는 'https'가 포함된 외부 링크를 필터링하여 출력했습니다.")

# 5. 'traffic' 또는 '교통'이 포함된 텍스트 찾기
traffic_texts = soup.find_all(string=lambda string: any(keyword in string.lower() for keyword in ['_____', '_____']) if string else False)  # 'traffic' 또는 '교통'이 포함된 텍스트 찾기
