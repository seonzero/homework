...
BUS_API_KEY = "6d794168737a6c7a393779664f5a61"  # 서울시 버스 교통 데이터 API 키 (빈칸 채우기)
SUBWAY_API_KEY = "536f6d77467a6c7a39356f7947566b"  # 서울시 지하철 교통 데이터 API 키 (빈칸 채우기)
...
def get_bus_data(date):
    ...
    for row in root.findall('.//row'):
        bus_data.append({
            ...
            '노선번호': row.find('RTE_NM'),  # row에서 'RTE_NM' 태그의 텍스트 추출)
            '정류장명': row.find('STTN_NM'),  # row에서 'STTN_NM' 태그의 텍스트 추출)
...
def get_subway_data(date):
    ...
    for row in root.findall('.//row'):
        subway_data.append({
            ...
            '역명': row.find('SBWY_STNS_NM'),  # row에서 'SBWY_STNS_NM' 태그의 텍스트 추출)
...
def integrate_data(bus_data, subway_data):
    ...
    # subway_data에서 '역명' 컬럼을 '역/정류장명'으로 변경)
