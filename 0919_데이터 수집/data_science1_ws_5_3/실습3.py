...
for i, table in enumerate(tables):
    ...
    table_name = title.get_text().strip() if title else f"Table {i + 1}"  
    # 테이블 이름을 설정합니다.

    data = [[td.get_text().strip() for td in tr.find_all(['td', 'th'])] for tr in table.find_all('tr')]  
    # 테이블의 모든 셀 내용을 추출합니다.
